...
# --- 2. Create a Custom PyTorch Dataset Class ---
class FashionDataset(Dataset):
    def __init__(self, dataframe, img_dir, processor, label_map):
...
if __name__ == '__main__':
    ...
    # Load the full dataset metadata
    full_df = pd.read_csv(CSV_PATH, on_bad_lines='skip')
    
    # --- NEW: Create a consistent label mapping ---
    all_labels = full_df['masterCategory'].unique()
    label_to_id = {label: i for i, label in enumerate(all_labels)}
    id_to_label = {i: label for i, label in enumerate(all_labels)}

    # --- NEW: Split the data into training and validation sets (80/20 split) ---
    ...
    train_dataset = FashionDataset(dataframe=train_df, img_dir=IMG_DIR, processor=processor, label_map=label_to_id)
    val_dataset = FashionDataset(dataframe=val_df, img_dir=IMG_DIR, processor=processor, label_map=label_to_id)
    ...
    # --- NEW: Updated Training Arguments ---
    training_args = TrainingArguments(
        output_dir='./fashion_model_checkpoints',
    per_device_train_batch_size=16,
    num_train_epochs=5,
    save_steps=1000,
    save_total_limit=2,
    logging_dir='./training_logs',
    report_to="none",
    eval_strategy="epoch",      # <-- THIS IS THE CORRECTED LINE
    logging_strategy="epoch" # <-- Logs training loss after each epoch
    )
    ...
    # --- NEW: Pass the validation dataset to the Trainer ---
    trainer = Trainer(
        model=model,
        args=training_args,
